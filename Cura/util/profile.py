...
setting('support',                _('None'), [_('None'), _('Touching buildplate'), _('Everywhere')], 'basic', _('Support')).setLabel(_('Support type'), _('Type of support structure build.\n"Touching buildplate" is the most commonly used support setting.\n\nNone does not do any support.\nTouching buildplate only creates support where the support structure will touch the build platform.\nEverywhere creates support even on top of parts of the model.'))
setting('platform_adhesion',      _('None'), [_('None'), _('Brim'), _('Raft')], _('basic'), _('Support')).setLabel(_('Platform adhesion type'), _('Different options that help in preventing corners from lifting due to warping.\nBrim adds a single layer thick flat area around your object which is easy to cut off afterwards, and the recommended option.\nRaft adds a thick raster at below the object and a thin interface between this and your object.\n(Note that enabling the brim or raft disables the skirt)'))
setting('support_dual_extrusion',  _('Both'), [_('Both'), _('First extruder'), _('Second extruder')], 'basic', _('Support')).setLabel(_('Support dual extrusion'), _('Which extruder to use for support material, for break-away support you can use both extruders.\nBut if one of the materials is more expensive then the other you could select an extruder to use for support material. This causes more extruder switches.\nYou can also use the 2nd extruder for soluble support materials.'))
...
#setting('extra_base_wall_thickness', 0.0, float, 'expert',   _('Accuracy')).setRange(0).setLabel(_('Extra Wall thickness for bottom/top (mm)'), _('Additional wall thickness of the bottom and top layers.'))
#setting('sequence', _('Loops > Perimeter > Infill'), [_('Loops > Perimeter > Infill'), _('Loops > Infill > Perimeter'), _('Infill > Loops > Perimeter'), _('Infill > Perimeter > Loops'), _('Perimeter > Infill > Loops'), _('Perimeter > Loops > Infill')], 'expert', _('Sequence'))
#setting('force_first_layer_sequence', True, bool, 'expert', _('Sequence')).setLabel(_('Force first layer sequence', 'This setting forces the order of the first layer to be \'Perimeter > Loops > Infill\''))
#setting('infill_type', _('Line'), [_('Line'), _('Grid Circular'), _('Grid Hexagonal'), _('Grid Rectangular')], 'expert', _('Infill')).setLabel(_('Infill pattern'), _('Pattern of the none-solid infill. Line is default, but grids can provide a strong print.'))
...
#setting('joris', False, bool, 'expert', _('Joris')).setLabel(_('Spiralize the outer contour'), _('[Joris] is a code name for smoothing out the Z move of the outer edge. This will create a steady Z increase over the whole print. It is intended to be used with a single walled wall thickness to make cups/vases.'))
#setting('bridge_speed', 100, int, 'expert', _('Bridge')).setRange(0,100).setLabel(_('Bridge speed (%)'), _('Speed at which layers with bridges are printed, compared to normal printing speed.'))
...
#setting('hop_on_move', False, bool, 'expert', _('Hop')).setLabel(_('Enable hop on move'), _('When moving from print position to print position, raise the printer head 0.2mm so it does not knock off the print (experimental).'))
...
setting('start.gcode', _(""";Sliced at: {day} {date} {time}
...
"""), str, 'alteration', 'alteration')
#######################################################################################
setting('start2.gcode', _(""";Sliced at: {day} {date} {time}
;Basic settings: Layer height: {layer_height} Walls: {wall_thickness} Fill: {fill_density}
;Print time: {print_time}
;Filament used: {filament_amount}m {filament_weight}g
;Filament cost: {filament_cost}
...
"""), str, 'alteration', 'alteration')
#######################################################################################
setting('end2.gcode', _(""";End GCode
...
#######################################################################################
setting('nextobject.gcode', _(""";Move to next object on the platform. clear_z is the minimal z height we need to make sure we do not hit any objects.
G92 E0
...
"""), str, 'alteration', 'alteration')
#######################################################################################
setting('switchExtruder.gcode', _(""";Switch between the current extruder and the next extruder, when printing with multiple extruders.
G92 E0
G1 E-36 F5000
G92 E0
T{extruder}
...
setting('machine_width', '205', float, 'preference', 'hidden').setLabel(_('Maximum width (mm)'),_( 'Size of the machine in mm'))
setting('machine_depth', '205', float, 'preference', 'hidden').setLabel(_('Maximum depth (mm)'),_( 'Size of the machine in mm'))
setting('machine_height', '200', float, 'preference', 'hidden').setLabel(_('Maximum height (mm)'),_( 'Size of the machine in mm'))
...
setting('has_heated_bed', 'False', bool, 'preference', 'hidden').setLabel(_('Heated bed'),_( 'If you have an heated bed, this enabled heated bed settings (requires restart)'))
setting('machine_type', 'unknown', str, 'preference', 'hidden')
setting('gcode_flavor', 'RepRap (Marlin/Sprinter)', ['RepRap (Marlin/Sprinter)', 'UltiGCode'], 'preference', 'hidden').setLabel(_('GCode Flavor'),_( 'Flavor of generated GCode.\nRepRap...'))
setting('extruder_amount', '1', ['1','2','3','4'], 'preference', 'hidden').setLabel(_('Extruder count'),_( 'Amount of extruders in your machine.'))
setting('extruder_offset_x1', '-21.6', float, 'preference', 'hidden').setLabel(_('Offset X'),_( 'The offset of your secondary extruder compared to the primary.'))
setting('extruder_offset_y1', '0.0', float, 'preference', 'hidden').setLabel(_('Offset Y'),_( 'The offset of your secondary extruder compared to the primary.'))
setting('extruder_offset_x2', '0.0', float, 'preference', 'hidden').setLabel(_('Offset X'),_( 'The offset of your secondary extruder compared to the primary.'))
setting('extruder_offset_y2', '0.0', float, 'preference', 'hidden').setLabel(_('Offset Y'),_( 'The offset of your secondary extruder compared to the primary.'))
setting('extruder_offset_x3', '0.0', float, 'preference', 'hidden').setLabel(_('Offset X'),_( 'The offset of your secondary extruder compared to the primary.'))
setting('extruder_offset_y3', '0.0', float, 'preference', 'hidden').setLabel(_('Offset Y'),_( 'The offset of your secondary extruder compared to the primary.'))
...
setting('steps_per_e', '0', float, 'preference', 'hidden').setLabel(_('E-Steps per 1mm filament'),_( 'Amount of steps per mm filament extrusion. If set to 0 then this value is ignored and the value in your firmware is used.'))
setting('serial_port', 'AUTO', str, 'preference', 'hidden').setLabel(_('Serial port'),_( 'Serial port to use for communication with the printer'))
setting('serial_port_auto', '', str, 'preference', 'hidden')
setting('serial_baud', 'AUTO', str, 'preference', 'hidden').setLabel(_('Baudrate'),_( 'Speed of the serial port communication\nNeeds to match your firmware settings\nCommon values are 250000, 115200, 57600'))
...
setting('extruder_head_size_min_x', '0.0', float, 'preference', 'hidden').setLabel(_('Head size towards X min (mm)'),_( 'The head size when printing multiple objects, measured from the tip of the nozzle towards the outer part of the head. 75mm for an Ultimaker if the fan is on the left side.'))
setting('extruder_head_size_min_y', '0.0', float, 'preference', 'hidden').setLabel(_('Head size towards Y min (mm)'),_( 'The head size when printing multiple objects, measured from the tip of the nozzle towards the outer part of the head. 18mm for an Ultimaker if the fan is on the left side.'))
setting('extruder_head_size_max_x', '0.0', float, 'preference', 'hidden').setLabel(_('Head size towards X max (mm)'),_( 'The head size when printing multiple objects, measured from the tip of the nozzle towards the outer part of the head. 18mm for an Ultimaker if the fan is on the left side.'))
setting('extruder_head_size_max_y', '0.0', float, 'preference', 'hidden').setLabel(_('Head size towards Y max (mm)'),_( 'The head size when printing multiple objects, measured from the tip of the nozzle towards the outer part of the head. 35mm for an Ultimaker if the fan is on the left side.'))
setting('extruder_head_size_height', '0.0', float, 'preference', 'hidden').setLabel(_('Printer gantry height (mm)'),_( 'The height of the gantry holding up the printer head. If an object is higher then this then you cannot print multiple objects one for one. 60mm for an Ultimaker.'))
